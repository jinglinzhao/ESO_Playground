...
ax1 = subplot(20,1,1:5);
    power_order = zeros(length(order), sum(x_idx));
    for idx = 1:11
        power_order(idx,:) = mean(abs(coeff(2*idx, :))) * power_fre(2*idx, x_idx);
    end
    power_nor = sum(power_order) / sum(mean(abs(coeff(order+1, :)),2));
    plot(x(x_idx), power_nor, 'k', 'LineWidth', 1)
...
new_pos1 = pos1 +[0.00 0 -0.08 0]
set(ax1, 'Position', new_pos1) % set new po
    
ax2 = subplot(20,1,6:20);
