...
filename    = ['../', star, '/4-ccf_dat/', char(dat_name(1))];
A           = importdata(filename);
[aa, bb]    = FUNCTION_FFT_noise(A, 0.1);
size1       = int16(2^nextpow2(length(bb))/2);
FFT_power   = zeros(size1+1, N_FILE);
...
for n = 1:N_FILE
    ...
    [FFT_frequency, FFT_power(:, n)] = FUNCTION_FFT_noise(A, 0.1);
    
    if 0            % Plot line profile in frequency space
        h_fft = figure;
        hold on 
        plot(FFT_frequency(1:size1/2), FFT_power(1:size1/2, n))
        plot(FFT_frequency(1:size1/2), FFT_power(1:size1/2, n), '.', 'markers',12)
        hold off
...
[pxx_fft,f_fft] = plomb(FFT_power(1, :)', MJD - min(MJD), 0.5,  'normalized');
...
h = pcolor(X_fft,Y_fft, (Z_fft(:,x_idx)));
